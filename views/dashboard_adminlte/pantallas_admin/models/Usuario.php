...
class Usuario extends Conexion
{
	...
	=============================================*/
        ...
        public function guardarEnDb(){
            $query = "INSERT INTO `usuario`(`email`, `nombre`, `clave`, `imagen`, `telefono`, `estado`, `cambioContrasena`, `created_at`) VALUES (:email,:nombre,:clave,:imagen,:telefono,:estado,:cambioContrasena,now())";
         try {
             self::getConexion();
             $email=$this->getEmail();
        ...
        public function actualizarUsuario(){
            $query = "update usuario set nombre=:nombre,telefono=:telefono where id=:id and email=:email";
            try {
                self::getConexion();
                $id=$this->getId();
                $email=$this->getEmail();
        ...
        public function verificarExistenciaEmail(){
            $query = "SELECT email,id,nombre,telefono FROM usuario where email=:email and estado =1";
            try {
            self::getConexion();
            $resultado = self::$cnx->prepare($query);		
            $email= $this->getEmail();		
